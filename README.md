...
Для настройки и добавления окружающей среды в программу нужно в папке `neuro_gym/environs` создать свой класс унаследованный от `from neuro_gym import Environ`.
...
Свойства `id`, `name`, `params` можно найти на `gymnasium.farama.org`. Свойство `complexity` относится к числу скрытых нейронов, которые будут задействованы для игры. `number_input_neurons` размер входного вектора или число признаков, которые отдаются из переменной `observation` после каждого шага. `number_output_neurons` размер выходного вектора из нейросети. `calc_confidence` дает заработать дополнительное вознаграждение за уверенное действие сети. `update_vector` метод который преобразует выходной вектор нейронной сети в данные пригодные для передачи в метод игры `action`.
